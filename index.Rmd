...
You'll be introduced in a following lesson with the raster package and the raster classes that have been developped as part of that package. All objects that belong to the class `raster*` are no more than vectors with a few more spatial attributes. *Note that in that case vectors are not related to spacial vector data; in R we call a collection of number a vector.* As a consequence, they can be handled as such, and handling vector is a crucial skill for processing raster data in R.
