...
```{block type="alert alert-info"}
**Tip**: You can use relative paths to load your data. 
...
You have seen in a [previous tutorial](https://geoscripting-wur.github.io/RPythonBasics/#data-types) already that every variable in your *R* working environment belongs to a class. You can take advantage of that, using control flow, to make your functions more flexible. First, let's introduce a new class.
...
- To force them to catch every error, check the box *Breakpoints - Errors* in the main menu, bottom left.

<center>
![file name](figs/breakpoint_errors.png)
</center>

- Run again ``foo(2)``.
- Positron will stop the execution where the error happened. The traceback will be highlighted in the code.
- You can use the little blue "Continue (F5)" button to go line by line through the code, or press the red "Disconnect (Shift + F5)" button to leave the debugging mode.
- Reset the *Breakpoints - Errors* behaviour by unchecking the box. In this default setting, Positron offers the option to traceback the error with a "Show Traceback" button in the console, when pressed takes you to the origin of the error.

<center>
![](figs/debugging_traceback_img.png){width=100%}
</center>
